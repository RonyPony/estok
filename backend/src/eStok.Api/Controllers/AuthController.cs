...
public sealed class AuthController(IAuthService auth, IValidator<RegisterRequest> validator, IConfiguration config) : ControllerBase
{
    ...
    [HttpPost("login")]
    public async Task<IActionResult> Login(LoginRequest request, CancellationToken ct) => Session(await auth.LoginAsync(request, ct));
    [HttpPost("refresh")]
    ...
    private CookieOptions CookieOptions() => new() { HttpOnly = true, Secure = Request.IsHttps, SameSite = SameSiteMode.Strict, Path = "/api/auth", Expires = DateTimeOffset.UtcNow.AddDays(30) };
