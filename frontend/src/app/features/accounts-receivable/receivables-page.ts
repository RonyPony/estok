...
@Component({selector:'app-receivables-page',imports:[AppPageHeader,AppTable],template:'<app-page-header [title]="payments?\'Pagos recibidos\':\'Cuentas por cobrar\'" subtitle="Una visión clara de tus ingresos y saldos." /><section class="card"><app-table [rows]="rows()" [columns]="columns" [total]="total()" [page]="page()" [loading]="loading()" (pageChange)="load($event)" actionLabel="Ver venta" (selected)="open($event)" /></section>'})
