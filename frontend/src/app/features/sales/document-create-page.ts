...
@Component({
 ...
 imports: [ReactiveFormsModule,RouterLink,MatButtonModule,MatInputModule,MatSelectModule,AppPageHeader,CustomerSelectorComponent,ProductSelectorComponent,SaleItemsComponent,SaleTotalsComponent,PaymentSectionComponent,HasPermission],
 template: `
 ...
 <form class="card document-editor" [formGroup]="form" (ngSubmit)="save()">
   ...
   @if(form.controls.items.length){<app-sale-items [items]="form.controls.items" (remove)="form.controls.items.removeAt($event)" />}
   ...
   <app-sale-totals [subtotal]="subtotal()" [discount]="discount()" [tax]="tax()" />
   @if(!quote){
...
export class DocumentCreatePageComponent {
 ...
 readonly form=this.fb.group({customerId:[''],warehouseId:[''],notes:['',Validators.maxLength(500)],items:new FormArray<LineForm>([])});
 ...
 productIds(){return this.form.controls.items.getRawValue().map(x=>x.productId);}
 ...
 addPending(){
   ...
   if(this.productIds().includes(p.id)){this.error.set('Este producto ya está agregado. Modifica su cantidad en la tabla.');return;}
   this.form.controls.items.push(this.fb.group({productId:[p.id],description:[p.name],quantity:[this.quantity.value,[Validators.required,Validators.min(.0001),Validators.max(1000000)]],discount:[0,Validators.min(0)],price:[p.salePrice],taxRate:[p.taxRate]}));
 ...
 total(){return Math.round((this.subtotal()-this.discount()+this.tax())*100)/100;}
 ...
 save(){
   ...
   const request={...values,customerId:!this.quote&&this.cashOnly()?null:values.customerId,payments:payment.amount>0?[payment]:[]};
