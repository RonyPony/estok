...
export interface Sale {id:string;customerId:string;warehouseId:string;saleNumber:string;status:string;total:number;subtotal:number;discount:number;tax:number;balance:number;paidAmount:number;paymentStatus:string;items:DocumentLine[];}
export interface DocumentRequest {customerId:string;warehouseId:string;notes:string;items:{productId:string;quantity:number;discount:number}[];}
