...
export class DocumentDetailPageComponent {
 ...
 download(){this.downloading.set(true);const action=this.quote?this.quotes.pdf(this.id):this.sales.pdf(this.id);action.subscribe({next:blob=>{const url=URL.createObjectURL(blob);const link=window.document.createElement("a");link.href=url;link.download=this.title()+".pdf";link.click();setTimeout(()=>URL.revokeObjectURL(url),1000);this.downloading.set(false);},error:()=>this.downloading.set(false)});}
